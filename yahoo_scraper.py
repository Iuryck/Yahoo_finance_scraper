...
#Main function, receives arguments in terminal


def main(argv):

    #If there is no arguments passed, shows usable arguments and exits    
    if len(argv) == 0:
        print ('\nyahoo_scraper.py -i <stock/list/file/path.csv>')
        print ('\nyahoo_scraper.py -h for help')
        sys.exit(2)


    #File path for the list of stocks file
    inputfile = ''
  
    #Passing arguments through terminal
    try:
        arguments, values = getopt.getopt(argv,"hi:",["help","ifile="])
   ...
   #Function for each argument
    for opt, arg in arguments:
        ...
        if opt in ("-h", "--help"):
            ...
            #Help message
            print ('''\tUsage: yahoo_scraper.py -i <stock/list/file/path.csv>\n\n
        ...
        #Usage with input csv file
        elif opt in ("-i", "--ifile"):
            inputfile = arg
            
            #Runs script with the stocks list
            get_tickers(inputfile)
        
        #If any other argument passes
        else:
            print('\ninvalid arguments')
            print ('\nyahoo_scraper.py -i <stock/list/file/path.csv>')
            print ('yahoo_scraper.py -h for help')
            sys.exit(2)
